...
function receiptShell({ greetingName }) {
...
<div style="max-width:480px;margin:0 auto;background:#fff;border-radius:20px;overflow:hidden;box-shadow:0 4px 32px rgba(0,0,0,.08)">
  ...
  <div style="padding:32px">
    ...
    <p style="color:#94a3b8;font-size:13px;line-height:1.6;margin-bottom:24px">📎 The full receipt is attached to this email as a PDF.</p>
...
export function contractorReceiptEmail({ contractorName, tradeName, siteName }) {
  const subject = `🧾 Payment Receipt — ${tradeName} · ${siteName} — TradeLink`;
  const html = receiptShell({ greetingName: contractorName ?? 'Contractor' });
  return { subject, html };
}

// Sent to the trade pro.
export function tradeReceiptEmail({ tradeName, siteName }) {
  const subject = `🧾 Payment Receipt — ${siteName} — TradeLink`;
  const html = receiptShell({ greetingName: tradeName });
  return { subject, html };
}
